...
from Rosenbrock_function import rosenbrock_2
...
class Bees:
    def __init__(self, func, scouts, elite, perspect, bees_to_leet, bees_to_persp, radius, position_x, position_y):
        ...
        self.scouts = [[random.uniform(-self.pos_x, self.pos_x), random.uniform(-self.pos_y, self.pos_y), 0.0] for _ in range(scouts)]
    ...
    def selected_search(self, param):
        for i in range(self.e):
            Bees.send_workers(self.func, self.workers[i*self.b_leet:i*self.b_leet+self.b_leet], self.selected[i], self.rad*param)

        for i in range(self.p):
            Bees.send_workers(self.func, self.workers[self.e*self.b_leet + i*self.b_persp:self.e*self.b_leet + i*self.b_persp + self.b_persp], self.selected[self.e+i], self.rad * param)
